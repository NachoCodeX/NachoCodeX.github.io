...
class SieveOFEratosthenes {
  ...
  initListeners() {
    this.input.addEventListener(
      "input",
      _.debounce(this.onChanges.bind(this), 500)
    );
  ...
  generateGridStyles() {
    ...
    if (this.total >= 20 && this.total < 30)
      this.board.style.gridTemplateColumns = `repeat(5,1fr)`;
    else if (this.total >= 30 && this.total < 40)
      this.board.style.gridTemplateColumns = `repeat(6,1fr)`;
    else if (this.total >= 40 && this.total < 50)
      this.board.style.gridTemplateColumns = `repeat(8,1fr)`;
    else {
